...
return function()
  ...
  function busted.rejectAll(element)
    busted.reject('randomize', element)
    for descriptor, _ in pairs(busted.executors) do
      busted.reject(descriptor, element)
    end
  end
  ...
  function busted.execAll(descriptor, current, propagate, err)
    local parent = busted.context.parent(current)

    if propagate and parent then
      local success, ancestor = busted.execAll(descriptor, parent, propagate)
      if not success then
  ...
  function busted.dexecAll(descriptor, current, propagate, err)
    local parent = busted.context.parent(current)
    local list = current[descriptor] or {}
    ...
    if propagate and parent then
      if not busted.dexecAll(descriptor, parent, propagate) then
        success = nil
      end
    end
    return success
