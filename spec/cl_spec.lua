...
describe('Tests the busted command-line options', function()
  ...
  it('tests running with --exclude-tags specified', function()
    local success, exitcode
    error_start()
    success, exitcode = execute('bin/busted --pattern=_tags.lua$ --exclude-tags=tag1,tag2,dtag1,dtag2')
    assert.is_true(success)
    assert.is_equal(0, exitcode)
    success, exitcode = execute('bin/busted --pattern=_tags.lua$ --exclude-tags=tag2,dtag1,dtag2')
    assert.is_false(success)
  ...
  it('tests running with --filter specified', function ()
    local success, exitcode
    error_start()
    success, exitcode = execute('bin/busted --pattern=_filter.lua$')
    assert.is_false(success)
    assert.is_equal(8, exitcode)
  ...
  it('tests running with --filter-out specified', function ()
    local success, exitcode
    error_start()
    success, exitcode = execute('bin/busted --pattern=_filter.lua$ --filter-out="pattern1"')
    assert.is_false(success)
    assert.is_equal(6, exitcode)
    success, exitcode = execute('bin/busted --pattern=_filter.lua$ --filter-out="pattern%d"')
    assert.is_false(success)
    assert.is_equal(5, exitcode)
    success, exitcode = execute('bin/busted --pattern=_filter.lua$ --filter-out="patt.*(%d)"')
    assert.is_true(success)
    error_end()
  end)
  ...
  it('tests running with --filter and --filter-out specified', function ()
    local success, exitcode
    error_start()
    success, exitcode = execute('bin/busted --pattern=_filter.lua$ --filter="pattern3" --filter-out="patt.*[12]"')
    assert.is_true(success)
    error_end()
  end)
...
describe('Tests no-keep-going commandline option', function()
  it('skips all tests after first error', function()
    local success, exitcode
    error_start()
    success, exitcode = execute('bin/busted --no-keep-going --pattern=cl_two_failures.lua$')
    assert.is_false(success)
    assert.is_equal(1, exitcode)
    error_end()
  end)
