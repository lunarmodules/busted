...
describe('Tests the busted command-line options', function()
  ...
  it('tests running a configfile for luacov', function()
    ...
    local success, errcnt = executeBusted('--coverage-config-file=' .. custom_config .. ' --pattern=cl_execute_fail.lua$')
    assert.is_false(success)
    assert.is_equal(1, errcnt)
    assert.is_false(path.isfile(custom_report))
    success, errcnt = executeBusted('--coverage --coverage-config-file=' .. custom_config .. ' --pattern=cl_execute_fail.lua$')
    assert.is_false(success)
    assert.is_equal(1, errcnt)
