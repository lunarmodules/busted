...
describe('Tests the busted command-line options', function()
  it('tests running with --tags specified', function()
    ...
    success, exitcode = executeBusted('--pattern=_tags.lua$ --tags=tag1,tag2')
    assert.is_false(success)
    assert.is_equal(4, exitcode)
    success, exitcode = executeBusted('--pattern=_tags.lua$ --tags=tag1 --tags=tag2')
    assert.is_false(success)
    assert.is_equal(4, exitcode)
  end)
  ...
  it('tests running with --exclude-tags specified', function()
    ...
    success, exitcode = executeBusted('--pattern=_tags.lua$ --exclude-tags=tag2 --exclude-tags=dtag1,dtag2')
    assert.is_false(success)
    assert.is_equal(2, exitcode)
  end)
  ...
  it('tests running with --filter specified', function ()
    ...
    assert.is_false(success)
    assert.is_equal(3, exitcode)
    success, exitcode = executeBusted('--pattern=_filter.lua$ --filter="pattern2"')
    assert.is_false(success)
    assert.is_equal(2, exitcode)
    success, exitcode = executeBusted('--pattern=_filter.lua$ --filter="pattern1" --filter="pattern2"')
    assert.is_false(success)
    assert.is_equal(4, exitcode)
  end)
  ...
  it('tests running with --filter-out specified', function ()
    ...
    assert.is_false(success)
    assert.is_equal(5, exitcode)
    success, exitcode = executeBusted('--pattern=_filter.lua$ --filter-out="patt1" --filter-out="patt2"')
    assert.is_false(success)
    assert.is_equal(3, exitcode)
    success, exitcode = executeBusted('--pattern=_filter.lua$ --filter-out="patt.*(%d)"')
    assert.is_true(success)
  end)
  ...
  it('tests running with --lazy specified', function()
    ...
    assert.is_false(success)
    assert.is_equal(2, exitcode)
    success, exitcode = executeBusted('--lazy --pattern=_tags.lua$ --tags=tag1,tag2')
    assert.is_false(success)
    assert.is_equal(3, exitcode)
    success, exitcode = executeBusted('--lazy --pattern=_tags.lua$ --tags=tag1 --tags=tag2')
    assert.is_false(success)
    assert.is_equal(3, exitcode)
  end)
...
describe('Test busted running standalone', function()
  it('tests running with --tags specified', function()
    ...
    assert.is_false(success)
    assert.is_equal(2, exitcode)
    success, exitcode = executeLua('spec/cl_standalone.lua --tags=tag1,tag2')
    assert.is_false(success)
    assert.is_equal(3, exitcode)
  end)
...
describe('Test busted command-line runner', function()
  it('runs standalone spec', function()
    ...
    assert.is_false(success)
    assert.is_equal(2, exitcode)
    success, exitcode = executeBusted('--tags=tag1,tag2 spec/cl_standalone.lua')
    assert.is_false(success)
    assert.is_equal(3, exitcode)
  end)
end)
...
describe('Tests pending through the commandline', function()
  it('skips tests inside pending scope', function()
    ...
    success, exitcode = executeBusted('--pattern=cl_pending.lua$')
    assert.is_true(success)
    assert.is_equal(0, exitcode)
  end)
...
describe('Tests random seed through the commandline', function()
  it('test seed value', function()
    local success, exitcode
    success, exitcode = executeBusted('--seed=12345 --pattern=cl_random_seed.lua$')
    assert.is_true(success)
    assert.is_equal(0, exitcode)
  end)
...
describe('Tests shuffle commandline option', function()
  for _, opt in ipairs({ '--shuffle', '--shuffle-tests' }) do
    it('forces test shuffling for non-randomized tests, ' .. opt, function()
      local success, exitcode = executeBusted(opt .. ' --pattern=cl_randomize.lua$')
      assert.is_true(success)
      assert.is_equal(0, exitcode)
    end)
  end
...
describe('Tests Xhelper commandline option', function()
  ...
  it('forwards single option to helper script', function()
    local success, exitcode = executeBusted('--helper=spec/cl_helper_script.lua -Xhelper "--fail-before-each" --pattern=cl_success.lua$')
    assert.is_false(success)
    assert.is_equal(1, exitcode)
  end)

  it('forwards multiple options to helper script', function()
    local success, exitcode = executeBusted('--helper=spec/cl_helper_script.lua -Xhelper "--fail-before-each,--fail-after-each" --pattern=cl_success.lua$')
    assert.is_false(success)
    assert.is_equal(2, exitcode)
  end)

  it('forwards multiple options to helper script using multiple -Xhelper', function()
    local success, exitcode = executeBusted('--helper=spec/cl_helper_script.lua -Xhelper "--fail-before-each" -Xhelper "--fail-after-each" --pattern=cl_success.lua$')
    assert.is_false(success)
    assert.is_equal(2, exitcode)
  end)
end)
...
describe('Tests helper script', function()
  it('can add setup to test suite', function()
    local success, exitcode = executeBusted('--helper=spec/cl_helper_script.lua -Xhelper "--fail-setup" --pattern=cl_two_failures.lua$')
    assert.is_false(success)
    assert.is_equal(1, exitcode)
  end)

  it('can add teardown to test suite', function()
    local success, exitcode = executeBusted('--helper=spec/cl_helper_script.lua -Xhelper "--fail-teardown" --pattern=cl_two_failures.lua$')
    assert.is_false(success)
    assert.is_equal(3, exitcode)
  end)

  it('runs setup/teardown for mutiple runs', function()
    local success, exitcode = executeBusted('--helper=spec/cl_helper_script.lua -Xhelper "--fail-setup,--fail-teardown" --pattern=cl_success.lua$ --repeat=2')
    assert.is_false(success)
    assert.is_equal(4, exitcode)
  end)

  it('runs setup/teardown for mutiple runs with --lazy', function()
    local success, exitcode = executeBusted('--lazy --helper=spec/cl_helper_script.lua -Xhelper "--fail-setup,--fail-teardown" --pattern=cl_success.lua$ --repeat=2')
    assert.is_false(success)
    assert.is_equal(4, exitcode)
  end)
end)
