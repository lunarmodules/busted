...
describe('Tests helper script', function()
  ...
  it('can add setup and teardown to nested test suite', function()
    local success, exitcode = execute(busted_cmd .. ' --helper=spec/cl_helper_script.lua -Xhelper "--fail-teardown" --pattern=cl_two_failures_nested.lua$')
    assert.is_false(success)
    assert.is_equal(6, exitcode)
  end)
end)

describe('Test empty suite', function() 
  it('wont run setup', function()
    local success, exitcode = execute(busted_cmd .. ' --helper=spec/cl_helper_script.lua -Xhelper "--fail-setup" --pattern=cl_empty.lua$')
    -- assert.is_true(success)
    assert.is_equal(0, exitcode)
  end)

  it('wont run teardown', function()
    local success, exitcode = execute(busted_cmd .. ' --helper=spec/cl_helper_script.lua -Xhelper "--fail-teardown" --pattern=cl_empty.lua$')
    -- assert.is_true(success)
    assert.is_equal(0, exitcode)
  end)
end)
